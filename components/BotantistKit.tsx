    ...
    <div className="fixed bottom-0 left-0 w-full z-50 pointer-events-none flex justify-center pb-2 md:pb-8 px-2 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-500 fill-mode-both">
      <div className="pointer-events-auto flex flex-col items-center w-full max-w-3xl">
        ...
        >
          ...
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse ml-0.5 md:ml-1"></span>
          <div className="ml-2 text-slate-500 group-hover:text-emerald-400 transition-colors">
            {isCollapsed ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
          </div>
        </button>

        {/* The Dock */}
        <div className={`bg-slate-900/95 backdrop-blur-xl border border-slate-700/50 rounded-2xl md:rounded-3xl shadow-[0_20px_40px_rgba(0,0,0,0.4)] transition-all duration-500 ease-in-out w-full overflow-hidden ${isCollapsed ? 'max-h-0 opacity-0' : 'max-h-96 opacity-100 p-2 md:p-4'}`}>
                ...
                return (
                  <button
                    ...
                    className={`
                      ...
                      ${isSelected 
                        ? 'bg-emerald-950/50 border border-emerald-500/50 shadow-[0_0_20px_rgba(16,185,129,0.2)] -translate-y-1.5 md:-translate-y-2' 
                        : 'bg-slate-800/50 border border-slate-700/50 hover:bg-slate-800 hover:border-slate-600 hover:-translate-y-1'}
                  ...
                  >
                    ...
                    <div className={`
                      ...
                      ${isSelected ? 'bg-emerald-500/20 scale-110' : 'bg-slate-900/50 group-hover:scale-110'}
                    `}>
                      {item.icon}
                    </div>
                    
                    <h3 className="font-bold text-slate-200 text-[10px] md:text-sm mb-0.5 md:mb-1">{item.name}</h3>
                    <span className="font-mono text-emerald-400 font-bold text-[9px] md:text-xs mb-1.5 md:mb-2">
                      ${item.price.toFixed(2)}
                    </span>
                    
                    {/* Tooltip / Extra Info */}
                    <div className={`
                      absolute -top-12 md:-top-14 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[9px] md:text-[10px] px-2 md:px-3 py-1.5 md:py-2 rounded-lg md:rounded-xl 
                      opacity-0 md:group-hover:opacity-100 transition-all duration-300 whitespace-nowrap border border-slate-600 shadow-xl pointer-events-none
                      flex flex-col items-center gap-0.5 md:gap-1
                      ${isSelected ? 'opacity-100 -translate-y-1.5 md:-translate-y-2' : ''}
                    `}>
                      <span className="font-bold text-emerald-400">Offsets {item.co2Factor}kg CO2/yr</span>
                      <span className="text-slate-400">Unlocks at Level {item.unlockLevel}</span>
                      <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1.5 h-1.5 md:w-2 md:h-2 bg-slate-800 border-b border-r border-slate-600 rotate-45"></div>
                    </div>
                  </button>
                );
              })}
